...
function altitude(){
    let alt=0
    for (const cmde of commandes) {
         if (cmde[1]=="monte") {
            alt+=  cmde[2]
         }else if(cmde[1]=="descend"){
            alt-=  cmde[2]
         }else{
            break
         } 
 ...
 * JS
 * Fonction maFonctionJavaScript  ==> camelCase
 * Objet MaFonctionJavaScript  ==> PascalCase
 * *****Autres Convention******
 * ma_fonction_java_script  ==> SnakeCase
 * ma.fonction.java.script  ==> KebabCase
 *   ==
 */

/**
 * Fonction Tableaux , CallBack 
 * array.callBack(()=>{
 * 
 * })
 */

coutMenu3=menu.reduce((cout,mnu)=>{
    return cout+ carte[mnu][1]
},0)
...
coutMenu1=function(){
  ...
  function isVegetarien(menu){
    menu.forEach((valeur,index) => {
        if (carte[valeur][2]==True) {
            return false
         }   
      });
    return true;
  ...
  isVegetarien1=menu.every(mnu=>carte[mnu][2]==false) 
